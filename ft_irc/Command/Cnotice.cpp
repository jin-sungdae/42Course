				...
				struct CommandChunk commandChunk)
{
	...
	if (target[0] == '#')
	{
		...
		{
			string msg = UserManager::makeMessage(user, RPL_NOTICE, targetChannel->getChannelName(), message);
	...
	else
	{
		...
		{
			string msg = UserManager::makeMessage(user, RPL_NOTICE, targetUser->getNickname(), message);
